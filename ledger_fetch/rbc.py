...
class RBCDownloader(BankDownloader):
    ...
    def fetch_transactions_for_account(self, account: Account, days: int = None) -> Optional[List[Transaction]]:
        """
        ...
        Args:
            ...
            days (int): Number of days of history to request. if None, uses config.
            
        Returns:
            Optional[List[Transaction]]: List of transactions if successful, None if failed/skipped.
        """
        # Determine days to fetch from config if not provided
        if days is None:
        ...
        # Check account type
        if account.type in [AccountType.CHEQUING, AccountType.SAVINGS]:
            return self._fetch_pda_transactions_search(account, days, encrypted_id)
        else:
            print(f"  Skipping API fetch for {account.type} (relying on CSV fallback)")
            return None

    def _fetch_pda_transactions_search(self, account: Account, days: int, encrypted_id: str) -> Optional[List[Transaction]]:
        """
        Fetch PDA (Chequing/Savings) transactions using the search endpoint with pagination.
        Handles date ranges in 1-month chunks.
        """
        ...
        while current_start < end_date:
            ...
            while True:
                ...
                try:
                    ...
                    raw_txns = data.get('transactionList') or data.get('transactions', [])
                    ...
                    # Update counts
                    count_returned = len(raw_txns)
                    total_matches = data.get('totalMatches') # Might be None
                    
                    print(f"    Fetched {count_returned} transactions (Offset: {offset}, Total Matches: {total_matches})")
                    
                    # Check if we are done with this chunk
                    if count_returned < limit:
                        break # Less than limit returned, so we are at the end
        ...
        print(f"  Total PDA transactions found: {len(all_transactions)}")
        return all_transactions

    def _fetch_cc_transactions_search(self, account: Account, days: int, encrypted_id: str) -> Optional[List[Transaction]]:
        """
        Fetch Credit Card transactions using the search endpoint with pagination.
        Handles date ranges in 1-month chunks to avoid timeouts/limits.
        """
        ...
        while current_start < end_date:
            ...
            # Use smaller chunks to be safe with limits
        ...
        print(f"  Total CC transactions found: {len(all_transactions)}")
        return all_transactions
